...
public class EndToEndBuyProduct {
	...
	public void initilize(String browser) throws IOException {
		
		b = new Base();
		driver = b.initialization( browser);
	...
	public void addToCartProduct() {
		...
		log.info("Current Temerature in applciation is: " +currentTemperature);
		
		if(Integer.parseInt(currentTemperature)<19) {
			
			lp.btnBuyMoisturizers().click();
			log.info("Selecting Moisturizers");
			...
			log.info("Lowest Aloe Price on Page is: " +minimumAloePrice);
		...
		else if(Integer.parseInt(currentTemperature)>34) {
